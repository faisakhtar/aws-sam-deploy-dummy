...
data "archive_file" "sampletf_archive" {
  ...
  output_path = "../terraform/packages/sampletf.zip"
}

data "archive_file" "sampletf2_archive" {
  type        = "zip"
  source_dir = "../functions/tf2/"
  output_path = "${path.module}/packages/sampletf2.zip"
}
...
resource "aws_lambda_function" "sampletf2" {
  ...
  filename         = "${path.module}/packages/sampletf2.zip"
...
resource "null_resource" "sam_metadata_aws_lambda_function_sampletf2" {
    triggers = {
        ...
        built_output_path = "${path.module}/packages/sampletf2.zip"
